/**
 * Katakana Module
 * Handles Katakana page functionality
 */

const KatakanaModule = {
  data: [],
  filteredData: [],
   ...
   */
  async loadData() {
    this.data = await DataService.getModuleData('katakana');
  ...
  setupEventListeners() {
    ...
    $('#btnFlashcard').on('click', () => {
      window.location.href = 'flashcards.html?type=katakana';
    });

    // Quiz button
    $('#btnQuiz').on('click', () => {
      window.location.href = 'quiz.html?type=katakana';
    });
  },
...
$(document).ready(function() {
  if ($('#cardsContainer').length && window.location.pathname.includes('katakana')) {
    KatakanaModule.init();
  }
});
